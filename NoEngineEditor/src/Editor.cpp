...
void NoEngineEditor::setup_main_control_menu()
{
	...
	if (ImGui::CollapsingHeader("Point Lights"))
	{
		...
		for (int i = 0; i < sizeof(check_pointLights); ++i)
		{
			...
			ImGui::InputFloat(("Constant##point" + number).c_str(), &pointLights_float[i][0]);
			ImGui::InputFloat(("Linear##point" + number).c_str(), &pointLights_float[i][1]);
			ImGui::InputFloat(("Quadratic##point" + number).c_str(), &pointLights_float[i][2]);
